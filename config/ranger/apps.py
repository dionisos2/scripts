...
class CustomApplications(Applications):
	def app_default(self, c):
		...
		if f.extension is not None:
			if f.extension in ('pdf', ):
				return self.either(c, 'editor', 'evince', 'zathura', 'apvlv')
			if f.extension == 'djvu':
				return self.either(c, 'evince')
			if f.extension in ('xml', 'ps'):
		...
		if f.video or f.audio:
			if f.video:
				c.flags += 'd'
			return self.either(c, 'smplayer', 'mplayer2', 'mplayer', 'vlc',
					'totem')
		...
		if f.image:
			if c.mode in (11, 12, 13, 14):
				...
				return self.either(c, 'mirage', 'sxiv', 'feh', 'eog')
	...
	def app_set_bg_with_feh(self, c):
		...
		arg = {1: '--scale-down', 11: '--bg-scale', 12: '--bg-tile', 13: '--bg-center',
				14: '--bg-fill'}
		if c.mode in arg:
	...
	def app_feh(self, c):
		...
		if c.mode is 0 and len(c.files) is 1: # view all files in the cwd
			...
			return 'feh', '--start-at',  c.file.basename, "-F", images
		return 'feh', c
